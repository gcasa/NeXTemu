...
static BOOL
NXTAddRegisterBank (NXTMemory *memory, NXTUInt32 base, NXTUInt32 length)
{
  ...
  if (self != nil)
    {
      ...
      _romRegion =
          [[NXTMemoryRegion alloc] initWithBaseAddress:NXT_ROM_BOOT_BASE
                                                length:NXT_ROM_SIZE
                                              readOnly:YES];
      _romAliasRegion =
          [[NXTMemoryRegion alloc] initWithBaseAddress:NXT_ROM_BASE
                                                length:NXT_ROM_SIZE
                                              readOnly:YES];
      ...
      if (_memory == nil || _romRegion == nil || _romAliasRegion == nil
          ...
          || ![_memory addRegion:_romAliasRegion]
          || ![_memory addRegion:_ramRegion])
        {
          [self release];
          return nil;
        }
      ...
      if (!NXTAddRegisterBank (_memory, 0x02000000U, 0x00005000U) || /* DMA */
          !NXTAddRegisterBank (_memory, 0x02005000U, 0x00009000U)
          || /* system */
          ...
          || !NXTAddRegisterBank (_memory, 0x02114000U, 0x00000020U)
          || /* SCSI */
          !NXTAddRegisterBank (_memory, 0x02114108U, 0x00000004U)
          || !NXTAddRegisterBank (_memory, 0x02116000U, 0x00000008U)
          || /* timer */
          !NXTAddRegisterBank (_memory, 0x02118000U, 0x00000014U)
          || /* serial */
          !NXTAddRegisterBank (_memory, 0x0211a000U, 0x00000004U)
          || /* timer */
          !NXTAddRegisterBank (_memory, 0x02200000U, 0x00010000U) || /* TMC */
          !NXTAddRegisterBank (_memory, 0x02210000U, 0x00010000U)
          || /* Turbo slot */
...
- (BOOL)attachDiskImageAtPath:(NSString *)path error:(NSString **)errorMessage
{
  ...
  if (attributes == nil
    ...
    {
      if (errorMessage != NULL)
        *errorMessage = @"Unable to read disk image";
      return NO;
    }
  ...
  if (size == 0 || (size & 511U) != 0)
    {
      if (errorMessage != NULL)
        *errorMessage = @"Disk image must be a non-empty raw image with "
                        @"512-byte sectors";
      return NO;
    }
  [path retain];
...
- (BOOL)loadROMAtPath:(NSString *)path error:(NSString **)errorMessage
{
  ...
  if (data == nil)
    {
      if (errorMessage != NULL)
        *errorMessage = @"Unable to read ROM image";
      return NO;
    }
  if ([data length] != NXT_ROM_SIZE)
    {
      if (errorMessage != NULL)
        *errorMessage = @"ROM image must be exactly 128 KiB";
      return NO;
    }
  if ([_memory loadData:data atAddress:NXT_ROM_BOOT_BASE] != NXTMemoryResultOK
    ...
    {
      if (errorMessage != NULL)
        *errorMessage = @"Unable to map ROM image";
      return NO;
    }
  return YES;
}
...
- (BOOL)reset
{
  ...
  if ([_memory writeLong:scr1 atAddress:0x0200c000U] != NXTMemoryResultOK ||
      ...
      || (_model == NXTMachineModelNeXTcubeTurbo &&
          [_memory writeLong:0x0fff4fafU atAddress:0x02200000U]
              != NXTMemoryResultOK))
    return NO;
...
{
  [_memory setVerboseBoot:verbose];
}
