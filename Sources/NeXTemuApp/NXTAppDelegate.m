...
- (void)drawRect:(NSRect)dirtyRect
{
  ...
  if (_memory == nil)
    return;
  framebuffer = [_memory regionContainingAddress:0x0c000000U
                                          length:288U * 832U];
  if (framebuffer == nil)
    framebuffer = [_memory regionContainingAddress:0x0b000000U
                                            length:288U * 832U];
  if (framebuffer == nil)
    return;
...
static NSTextField *
NXTCreateLabel (NSRect frame, NSString *text)
{
  NSTextField *field;
...
- (void)buildWindow
{
  ...
  _window =
      [[NSWindow alloc] initWithContentRect:NSMakeRect (100, 100, 760, 620)
                                  styleMask:style
  ...
  titleField
      = NXTCreateLabel (NSMakeRect (24, 174, 420, 26), @"NeXTcube Emulator");
  ...
  _modelButton =
      [[NSPopUpButton alloc] initWithFrame:NSMakeRect (84, 136, 220, 30)
                                 pullsDown:NO];
  ...
  _verboseButton = [[[NSButton alloc]
      initWithFrame:NSMakeRect (324, 138, 150, 24)] autorelease];
  ...
  [_verboseButton setState:[[NSUserDefaults standardUserDefaults]
                               boolForKey:NXTVerboseBootDefaultsKey]
                               ? NSOnState
                               : NSOffState];
  ...
  openButton = [[[NSButton alloc] initWithFrame:NSMakeRect (570, 106, 166, 30)]
      autorelease];
  ...
  _romField = [NXTCreateLabel (NSMakeRect (24, 110, 530, 22),
                               @"ROM: No ROM selected") retain];
  [_romField setLineBreakMode:NSLineBreakByTruncatingMiddle];
  [contentView addSubview:_romField];

  _diskField = [NXTCreateLabel (NSMakeRect (24, 80, 350, 22),
                                @"No SCSI disk attached") retain];
  [_diskField setLineBreakMode:NSLineBreakByTruncatingMiddle];
  [contentView addSubview:_diskField];
  diskButton = [[[NSButton alloc] initWithFrame:NSMakeRect (570, 76, 166, 30)]
      autorelease];
  ...
  _framebufferScaleButton =
      [[NSPopUpButton alloc] initWithFrame:NSMakeRect (428, 78, 126, 30)
                                 pullsDown:NO];
  ...
  _displayScrollView =
      [[NSScrollView alloc] initWithFrame:NSMakeRect (24, 210, 712, 368)];
  ...
  _displayView =
      [[NXTDisplayView alloc] initWithFrame:NSMakeRect (0, 0, 1120, 832)];
  ...
  _statusField =
      [NXTCreateLabel (NSMakeRect (24, 48, 520, 24),
                       @"Stopped — choose a 128 KiB NeXT ROM image") retain];
  [contentView addSubview:_statusField];
  _registerField = [NXTCreateLabel (NSMakeRect (24, 20, 520, 24),
                                    @"SSP: —    PC: —") retain];
  [_registerField setFont:[NSFont userFixedPitchFontOfSize:12.0]];
  [contentView addSubview:_registerField];

  _speedField =
      [NXTCreateLabel (NSMakeRect (382, 50, 172, 22), @"Speed: —") retain];
  [_speedField setAlignment:NSRightTextAlignment];
  [contentView addSubview:_speedField];

  resetButton = [[[NSButton alloc] initWithFrame:NSMakeRect (570, 28, 166, 32)]
      autorelease];
  ...
  [_diskField setAutoresizingMask:NSViewWidthSizable];
  {
    ...
    if (savedScale != 200 && savedScale != 300)
      savedScale = 100;
    [_framebufferScaleButton
        selectItemWithTitle:[NSString
                                stringWithFormat:@"%ld%%", (long)savedScale]];
...
- (void)openDiskImage:(id)sender
{
  ...
  if (_machine == nil)
    {
      NSRunAlertPanel (@"No Machine",
                       @"Load a ROM before attaching a disk image.", @"OK",
                       nil, nil);
      return;
    }
  ...
  if (![_machine attachDiskImageAtPath:path error:&errorMessage])
    {
      NSRunAlertPanel (@"Cannot Attach Disk", @"%@", @"OK", nil, nil,
                       errorMessage);
      return;
    }
  ...
  [_diskField
      setStringValue:[NSString
                         stringWithFormat:@"SCSI disk: %@ (%.1f MiB)", path,
                                          (double)[_machine diskImageSize]
                                              / (1024.0 * 1024.0)]];
  if (![_machine reset])
    {
      [_statusField
          setStringValue:
              @"SCSI disk attached, but the machine could not reset"];
      return;
    }
  [_statusField
      setStringValue:@"SCSI disk attached at target 0 — machine restarted"];
}
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification
{
  ...
  files = romsPath == nil ? nil
                          : [[NSFileManager defaultManager]
                                contentsOfDirectoryAtPath:romsPath
                                                    error:NULL];
  ...
  for (index = [files count]; index > 0; index--)
    {
      candidate = [romsPath
          stringByAppendingPathComponent:[files objectAtIndex:index - 1]];
      if ([self loadROMAtPath:candidate showErrors:NO])
        break;
    }
}
...
- (BOOL)loadROMAtPath:(NSString *)path showErrors:(BOOL)showErrors
{
  ...
  if ([[[path lastPathComponent] lowercaseString] rangeOfString:@"v74"]
              .location
          != NSNotFound
      || [[[path lastPathComponent] lowercaseString] rangeOfString:@"v72"]
                 .location
             != NSNotFound
      || [[[path lastPathComponent] lowercaseString] rangeOfString:@"rev_3"]
                 .location
             != NSNotFound)
    [_modelButton selectItemAtIndex:1];
  model = [_modelButton indexOfSelectedItem] == 0
              ? NXTMachineModelNeXTcube
  ...
  errorMessage = nil;
  if (newMachine == nil
      || ![newMachine loadROMAtPath:path error:&errorMessage])
    {
      if (errorMessage == nil)
        errorMessage = @"Unable to create the emulated machine";
      if (showErrors)
        NSRunAlertPanel (@"Cannot Open ROM", @"%@", @"OK", nil, nil,
                         errorMessage);
      [newMachine release];
      return NO;
    }
      ...
      if ([_machine attachDiskImageAtPath:savedDiskPath error:&errorMessage])
        {
          [_diskField
              setStringValue:[NSString
                                 stringWithFormat:@"SCSI disk: %@ (%.1f MiB)",
                                                  savedDiskPath,
                                                  (double)[_machine
                                                      diskImageSize]
                                                      / (1024.0 * 1024.0)]];
...
- (void)resetMachine:(id)sender
{
  ...
  if (_machine == nil)
    {
      NSBeep ();
      [_statusField
          setStringValue:@"Stopped — choose a 128 KiB NeXT ROM image"];
  ...
  if (![_machine reset])
    {
      [_statusField
          setStringValue:@"Reset failed: ROM vectors could not be read"];
      return;
    }
...
- (void)changeFramebufferScale:(id)sender
{
  ...
  NSRect windowFrame;
  CGFloat oldTop;
  if ([title hasPrefix:@"200"])
    scale = 200;
  else if ([title hasPrefix:@"300"])
    scale = 300;
  [[NSUserDefaults standardUserDefaults]
      setInteger:scale
          forKey:NXTFramebufferScaleDefaultsKey];
  [[NSUserDefaults standardUserDefaults] synchronize];
  framebufferSize = NSMakeSize (1120.0 * scale / 100.0, 832.0 * scale / 100.0);
  ...
  windowFrame.size =
      [_window frameRectForContentRect:NSMakeRect (0, 0, contentSize.width,
                                                   contentSize.height)]
          .size;
  windowFrame.origin.y = oldTop - windowFrame.size.height;
  [_window setFrame:windowFrame display:YES animate:NO];
  [_displayScrollView setFrame:NSMakeRect (24, 210, framebufferSize.width,
...
- (void)emulationTick:(NSTimer *)timer
{
  ...
  (void)timer;
  if (_machine == nil || [[_machine processor] isStopped])
    return;
  ...
  if (result == NXTProcessorResultStopped)
    ...
    {
      status =
          [NSString stringWithFormat:@"Running — %llu instructions",
                                     (unsigned long long)[[_machine processor]
    ...
    {
      status = [NSString
          stringWithFormat:@"Unsupported opcode %04x at %08x",
                           (unsigned int)[[_machine processor] lastOpcode],
                           (unsigned int)[[_machine processor]
                               lastOpcodeAddress]];
    }
  [_statusField setStringValue:status];
  registers = [NSString
      stringWithFormat:@"SSP: %08x    PC: %08x",
  ...
  if (elapsed >= 0.5)
    {
      ...
      relativeSpeed
          = ((double)(instructionCount - _speedSampleInstructions) / elapsed)
            / ((double)[_machine clockSpeedMHz] * 1000000.0);
      [_speedField
          setStringValue:[NSString stringWithFormat:@"Speed: %.0f%% real-time",
                                                    relativeSpeed * 100.0]];
      _speedSampleInstructions = instructionCount;
      _speedSampleTime = now;
    }
  if ((_displayTicks % 5) == 0)
    [_displayView setNeedsDisplay:YES];
}
